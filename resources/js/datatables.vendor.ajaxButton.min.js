...
window.__ibEditorValuesEqual = function (val, oldVal)
{
    ...
    if (val === '')
    {
        ...
        var oldNumZero = parseFloat(String(oldVal).replace(',', '.'));

        if (!isNaN(oldNumZero) && oldNumZero === 0)
            return true;
    }

    var valNum = parseFloat(String(val).replace(',', '.'));
    var oldNum = parseFloat(String(oldVal).replace(',', '.'));
...
window.__ibLogEditorDebug = function (target, extra)
{
    ...
    var $el = $(target);
    var val = $el.val();
    var dataOriginal = $el.data('originalvalue');
    var changed = !window.__ibEditorValuesEqual(val, dataOriginal);
...
window.editorFieldHasChanged = function (target)
{
    ...
    return !window.__ibEditorValuesEqual($t.val(), $t.data('originalvalue'));
};
...
// Optimistically clear dirty markers (server errors handled by existing error UI)
    ...
    $('body').on('input change', '.ib-editor-text', function(e)
    {
        ...
        let changed = !window.__ibEditorValuesEqual(current, original);
