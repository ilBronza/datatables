(function ilBronzaDatatablesFiltering() {
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex, rowData)
    {
        ...
        if (!api.getDataAttribute || !api.getDataAttribute('showSelected'))
            return true;

        return api.row(dataIndex).selected();
    });
    ...
    $(document).ready(function($)
    {
        ...
        $('body').on('change keyup input', '.columns input, .columns select, .columns textarea', function (e)
        {
            if (! e.target.closest('div.datatablefilter'))
                return;

            // toggle class on the input itself (keyup/input = subito, non solo dopo blur/invio su change)
            ...
            var tableId = table.getAttribute('data-realid');
            if (typeof window.ibSyncClearFiltersButtonForTable === 'function' && tableId) {
        ...
        $('body').on('change keyup blur', '.sectionheader input, .sectionfooter input, .sectionheader select, .sectionfooter select', function (e) {
            ...
            // 'enter' è un evento virtuale: equivale a keyup ma SOLO col tasto Invio
            var eventAllowed = events.indexOf(e.type) !== -1
                || (isEnter && events.indexOf('enter') !== -1);
